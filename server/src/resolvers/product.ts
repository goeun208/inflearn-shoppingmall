...
import { DocumentData, addDoc, collection, doc, getDoc, getDocs, limit, orderBy, query, serverTimestamp, startAfter, updateDoc, where } from 'firebase/firestore'
...
const productResolver: Resolver = {
    ...
    Mutation:  {
        ...
        deleteProduct: async (parent, { id }) => {
            // 실제 db에서 delete하는 대신, createdAt을 지워준다!!
            const productRef = doc(db, 'products', id)
            if (!productRef) throw new Error('상품이 없습니다.')
